...
//=================================================================
 ...
 // organization MSP Id
 const orgMspId = 'CoffeeMSP';

 // Network configuration
 const networkYAML = './connection-profile.yaml';

 // Channel name
 const channelNAME = 'devchannel';

 // Peer name
 const peerName = 'peer0.coffee.blockchain.biz';
 //=================================================================
...
async function main() {
    ...
    try {
		...
		var certPath = '../coffee/rca/users/' + userName + '/msp/signcerts/cert.pem';
		var cert = fs.readFileSync(certPath).toString();
		var keystorePath = '../coffee/rca/users/' + userName + '/msp/keystore/key.pem';
